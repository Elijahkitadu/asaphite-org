...
export function ContactHero() {
  return (
    <PageHero
      ...
      subtitle="We are a small team in Dar es Salaam. We read every message and reply personally."
              ...
              {[
                ...
                <motion.div key={item.label} variants={fadeInUp} className="flex gap-4">
                  ...
                  <div className="flex flex-col gap-0.5">
                    ...
                    {item.href ? <a href={item.href} className="text-navy-800 font-medium text-sm hover:text-gold-400 transition-colors">{item.value}</a>
              ...
              <div className="flex flex-col gap-3">
                {SOCIAL_LIST.map(social => (
                  <motion.a key={social.label} variants={fadeInUp} href={social.href} target="_blank" rel="noopener noreferrer"
                    className="flex items-center gap-3 text-sm text-gray-500 hover:text-navy-800 transition-colors group">
                    <div className="flex items-center justify-center w-8 h-8 rounded-full bg-gray-100 text-gray-500 group-hover:bg-gold-400/10 group-hover:text-gold-400 transition-all duration-200">{social.icon}</div>
              ...
              {submitted ? (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    <p className="text-gray-500 max-w-sm">Thank you for reaching out. We will reply personally within 24–48 hours.</p>
                  </div>
                  <Button variant="outline-gold" size="md" onClick={() => { setSubmitted(false); setError('') }}>Send Another Message</Button>
                </motion.div>
              ) : (
                <form onSubmit={handleSubmit} className="flex flex-col gap-6" noValidate>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {[
                      ...
                      { id: 'c-phone',   label: 'Phone / WhatsApp', placeholder: '+255 000 000 000', type: 'tel',   required: false },
                      { id: 'c-email',   label: 'Email *',          placeholder: 'amina@email.com',  type: 'email', required: true  },
                      { id: 'c-subject', label: 'Subject *',        placeholder: 'How can we help?', type: 'text',  required: true  },
                    ].map(field => (
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <textarea id="c-message" name="c-message" rows={4} required placeholder="Tell us what is on your mind…"
