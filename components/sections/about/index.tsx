      ...
      <div className="container-site">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          <motion.div variants={fadeInRight} initial="hidden" whileInView="visible" viewport={viewport} className="relative hidden lg:block">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={viewport} transition={{ delay: 0.4, duration: 0.6 }}
...
const VALUE_ICONS: Record<string, React.ReactNode> = {
  Honesty: <Shield size={22} />, Dignity: <Heart size={22} />,
  Storytelling: <Film size={22} />, Community: <Users size={22} />, Hope: <Sun size={22} />,
}
    ...
    <section className="section-py bg-background" aria-labelledby="values-heading">
      <div className="container-site">
        <div className="flex flex-col items-center gap-12">
          <SectionTitle eyebrow="What We Stand For" title="Our values" subtitle="Simple principles that guide everything we do." id="values-heading" />
  ...
  return (
    <section className="section-py bg-navy-800 relative overflow-hidden" aria-labelledby="timeline-heading">
      ...
      <div className="container-site relative">
        <div className="flex flex-col items-center gap-12">
          <SectionTitle eyebrow="Our Journey" title="How we got here" subtitle="A short story — we are still writing it." light id="timeline-heading" />
          <div className="relative w-full max-w-2xl mx-auto">
            ...
            <div className="flex flex-col gap-0">
              {FOUNDATION_TIMELINE.map((item, i) => {
                const isEven = i % 2 === 0
                return (
                  <motion.div key={item.year} variants={fadeInUp} initial="hidden" whileInView="visible" viewport={viewport}
...
export function LeadershipSection() {
  return (
    <section className="section-py bg-white" aria-labelledby="team-heading">
      <div className="container-site">
        <div className="flex flex-col items-center gap-12">
          <SectionTitle eyebrow="The Team" title="The people behind the camera" subtitle="A small team with a shared passion for Tanzania and storytelling." id="team-heading" />
          <motion.div variants={staggerContainer} initial="hidden" whileInView="visible" viewport={viewport}
            className="grid grid-cols-1 sm:grid-cols-3 gap-8 w-full max-w-2xl mx-auto">
            {TEAM.map((member) => (
              <motion.div key={member.name} variants={fadeInUp} className="group flex flex-col items-center text-center gap-4">
                <div className="relative w-24 h-24 rounded-full overflow-hidden ring-4 ring-gray-100 group-hover:ring-gold-400/40 transition-all duration-300">
                  <Image src={member.image} alt={member.name} fill className="object-cover grayscale group-hover:grayscale-0 transition-all duration-500" sizes="96px" />
                </div>
