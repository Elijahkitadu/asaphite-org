...
interface CTASectionProps {
  ...
  className?:  string
  overlay?:    string  // tailwind gradient override
}
    ...
    >
      ...
      <div
        className={cn(
          ...
          overlay ?? 'bg-gradient-to-br from-navy-800/95 via-navy-800/88 to-navy-800/80'
        )}
      />

      {/* Dot pattern */}
      <div
        className="absolute inset-0 -z-10 bg-dot-pattern opacity-30"
        aria-hidden="true"
      />
      ...
      <div
        className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2
                   w-[600px] h-[300px] rounded-full -z-10
      ...
      <div className="container-site relative">
        <motion.div
          ...
          whileInView="visible"
          viewport={viewport}
          className="mx-auto max-w-3xl flex flex-col items-center text-center gap-6"
        >
          {eyebrow && (
            <motion.span
              variants={fadeInUp}
          ...
          <motion.h2
            ...
            className="font-heading font-bold text-white leading-tight tracking-tight
                       text-3xl sm:text-4xl lg:text-5xl text-balance"
          >
            {title}
          </motion.h2>
          ...
          >
            {links.map((link) => (
              <Button
                ...
                iconRight={link.variant !== 'outline' ? <ArrowRight size={18} /> : undefined}
              >
                {link.label}
              </Button>
            ))}
