...
import { Heart, Mail, MapPin, Phone, Facebook, Instagram, Twitter, Youtube, ShieldCheck } from 'lucide-react'
      ...
      <div className="container-site py-16 sm:py-20">
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-12">
          ...
          <div className="lg:col-span-4 flex flex-col gap-6">
            ...
            {/* ── Registration badge ── */}
            <div className="inline-flex items-center gap-3 px-4 py-3 rounded-xl bg-hope-500/10 border border-hope-500/25 w-fit">
            ...
            <div className="flex flex-col gap-3 text-sm text-white/55">
              <a href={`mailto:${SITE_EMAIL}`} className="flex items-center gap-2 hover:text-gold-400 transition-colors">
                <Mail size={15} />{SITE_EMAIL}
              </a>
              <a href={`tel:${SITE_PHONE}`} className="flex items-center gap-2 hover:text-gold-400 transition-colors">
                <Phone size={15} />{SITE_PHONE}
              </a>
              <span className="flex items-center gap-2">
                <MapPin size={15} />{SITE_LOCATION}
              </span>
            </div>
            ...
            {Object.entries(FOOTER_LINKS).map(([group, links]) => (
              <div key={group}>
                ...
                <ul className="flex flex-col gap-2.5" role="list">
                  {links.map((link) => (
                    <li key={link.label}>
                      <Link href={link.href} className="text-sm text-white/55 hover:text-gold-400 transition-colors duration-200">
                        {link.label}
                      </Link>
                    </li>
                  ))}
          ...
          <div className="lg:col-span-3 flex flex-col gap-5">
            ...
            <form className="flex flex-col sm:flex-row gap-3 max-w-sm" onSubmit={(e) => e.preventDefault()}>
              <input type="email" placeholder="Your email" required
                className="flex-1 px-4 py-2.5 rounded-full text-sm bg-white/8 border border-white/15 text-white placeholder:text-white/40 focus:outline-none focus:border-gold-400 transition-colors" />
              <button type="submit"
                className="shrink-0 px-5 py-2.5 rounded-full text-sm font-semibold font-heading bg-gold-400 text-navy-800 hover:bg-yellow-400 transition-colors whitespace-nowrap">
                Subscribe
              </button>
            </form>
            <p className="text-white/30 text-xs">
              Want to support our work?{' '}
              <Link href="/contact" className="text-gold-400 hover:underline">Get in touch</Link>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="border-t border-white/8">
        <div className="container-site py-5 flex flex-col sm:flex-row items-center justify-between gap-3">
          ...
          <p className="text-xs text-white/25 flex items-center gap-1">
            Made with <Heart size={10} className="fill-gold-400 text-gold-400" /> in Tanzania
          </p>
        </div>
      </div>
    </footer>
